...
test("limit is not the provider's to apply — ranking needs every candidate", async () => {
  // Order the rows so the exact match comes LAST. A provider that truncates
  // during the scan returns the two that merely share a token and never
  // reaches "San Jose" at all; one that returns everything lets the ranker
  // above it pick correctly.
  const noise = (geonameId: number, name: string) => ({
    geonameId, name, aliases: [name.toLowerCase()], country: "ar", admin1: "",
    lat: 0, lon: 0, zone: "America/Argentina/Buenos_Aires", population: 900_000,
  });
  const late = bundled({
    asOf: "2026-08-06",
    cities: [
      ...
      { geonameId: 3, name: "San Jose", aliases: ["san jose"], country: "us", admin1: "CA", lat: 37.33939, lon: -121.89496, zone: "America/Los_Angeles", population: 1_026_908 },
    ],
  });
...
test("a place is reported under its best-matching alias, not its first", async () => {
  const p = bundled({
    asOf: "2026-08-06",
    cities: [
      { geonameId: 3_621_849, name: "San José", aliases: ["san josé", "san jose"], country: "cr", admin1: "08", lat: 9.93333, lon: -84.08333, zone: "America/Costa_Rica", population: 335_007 },
    ],
  });
...
export function bundled(opts: BundledOptions): GeocodeProvider {
  ...
  return {
    ...
    async search(q: GeocodeQuery): Promise<readonly GeocodeHit[]> {
      ...
      // different city.
      const best = new Map<GeocodeHit, { readonly score: number; readonly matched: string }>();
      for (const entry of bucket) {
        ...
        if (seen === undefined || score > seen.score) best.set(entry.hit, { score, matched: entry.key });
      }

      // `q.limit` is deliberately NOT applied here. Ranking is §6's job and it
      // needs the whole candidate set: a provider that truncates in index order
      // first has already thrown the answer away — `san jose` limited to three
      // returns three Argentine cities sharing only the token "san".
      return [...best].map(([hit, m]) => ({ ...hit, matched: m.matched, score: m.score }));
    },
  };
}
```
