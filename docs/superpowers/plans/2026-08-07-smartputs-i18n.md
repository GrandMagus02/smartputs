...
**P5 (Tasks 19–21) — landed 2026-08-08. The plan is complete.** Twelve commits, `29ac010`..`d12263f`. Tests 3702 → 3802 pass / 1 skip, parity 86/0, typecheck/check-deps/build clean, `check-size` unmoved (the same three known-OVER rows, byte-identical).

**The goal is met, and it is measurable rather than asserted.** An engine composed from `english` with **no vocabularies at all** reads `5 kg` and prints `5 kg`, and reads `2 km in m` and prints `2,000 m` — the kind works with the unit key as its only name, which is possible only because no kind holds a word. A German `Language` written inside a test file, against seams it did not shape, reads `10 Zentimeter` through a helper it did not write, declines `10 Zentrum`, and shares one alias index and one solver with English and Ukrainian; 468 rows of every-unit × every-language × every-`format` leak nothing.

**Four things execution found.**

1. **The plan named the wrong caller for Task 19, and the right answer was a different mechanism.** Task 19 says "`Tokenizer`'s word fold supplies the segmented run". The run is built in `lex.ts`; `resolve` is called from `pratt.ts` on single tokens. Widening `WordToken` was tried first and **the parity net rejected it**: `explain().tokens` hands out token objects verbatim and `parity.ts` snapshots them field by field, so a new property is a recorded public output and six fixture rows moved for no behaviour. Shipped instead as a module-level `WeakMap<WordToken, WordPosition>` in `lex.ts`, written by `recordRuns` and read by `runOf` — keyed on identity because identity is the only thing that survives the trip (the token array is rebuilt twice between `lex` and `pratt`).

2. **A run is adjacency, not segmentation.** `lex` cuts a letter chunk at the first space, so a `segment()` hook could never show `phraseAnalyzer` the "square" in "square metres". The rule shipped is: word tokens with nothing but spaces between them, checked with `input.slice(prev.end, next.start).trim()`. Runs are recorded *before* the folds, so `kg` in "twenty two kg" keeps a three-word run whose first two words became a number token — a run is a fact about what was typed.

3. **`complete()` leaked again, one layer down — the same path P4 fixed.** P4 fixed the case where the format language has words for a unit but not the matched spelling. The floor underneath still ended at the matched alias, commented as R2's unit-key floor: "a kind no installed language has spoken for". **That comment is true only for two languages.** On en+uk, `wordsFor(format, …) === undefined` is reachable for exactly one entry — `boolean:bool`, whose index key *is* the unit key — so no test could see it. German ships `length` and nothing else, which separates "the format language has no word" from "nobody has a word" for the first time, and in that gap the alias is English's word: **37 of 194 completion rows under `format: "de"` came back in English**. Fixed in `b9ca45b`; the floor is now `entry.unit`, matching what `unit-word.ts` always did. The lesson generalises past this bug: **a comment that says "no installed language" while testing one language is the shape to watch for.**

4. **`identity()` is a required incantation that fails silently.** A `Language` omitting it cannot reach its own aliases — and reaches all of them the moment any other installed language supplies one. The failure is a *wrong unit*, not an error. Pinned as a measurement in the third-language test.

**What this plan says that the shipped code does not do.** A reader of the task list will be misled on three points:

- **Task 20's table promises `phraseAnalyzer` a capability the parser does not have.** "Multi-word units … `"square metres"` → `m2`" reads as a shipped feature. The analyzer produces the correct reading; `pratt` reads one word per quantity, so `evaluate("10 square metres")` throws `Unknown unit "square"` before `metres` is ever resolved. Verified directly. The helper's own doc comment and a warning block in `locales.md` are honest about this; **this table was not, and is the thing to fix before anyone builds on it.** Making a multi-word quantity work is a parser change, not an analyzer one.
- **Task 19 Step 3's cache claim is narrower than it sounds.** "The common path keeps its one-entry-per-word cache" holds for numeric input and fails for any two adjacent words — spelled numbers included. `"twenty two kg"` is ordinary English and pays per-run. Measured: **+11.5% misses** over the parity corpus, +25% over its keystroke prefixes.
- **The P4 entry above says `complete()` was fixed and I6 checked.** It was fixed for what a two-language engine can produce. Finding 3 is the same path failing again under the third language P5 itself shipped.

**Left for a human, deliberately not decided.** `compoundSplitter` emits every satisfying tail at one flat weight, so two matching tails have nothing to break the tie: `Personenkilometer` yields `kilometer` and `meter` both at `-3` and `10 Personenkilometer` throws `AmbiguityError`. Verified directly. Two coherent fixes — emit only the longest tail (matching the doc's opening sentence; nothing green moves), or weight by tail length (keeps every reading, at the cost of the flat weight the doc pins) — and choosing is a contract decision. Its doc comment also contradicts itself and must be reconciled either way. Also still open: P4's `evaluate("1,5 кг")` → `15 kilograms` under an English format, documented but unpinned by any test; `explain()` naming one unit twice on a split tie; and error hints being many-locale regardless of `format`, which is generation-shaped output I6 does not cover.
