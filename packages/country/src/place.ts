 ...
 * this package imports `@smartput/city`'s data, deliberately — the one edge is
 * `@smartput/city/types`, which compiles away: a static import links the whole
 * gazetteer into every bundle that touches "@smartput/country", and no bundler
 * can prove `CITIES` unused once the kind closes over it. The tiering in spec §3
 * is only real if the dependency edge runs from the consumer inwards, so
 * `@smartput/city` is a package the consumer names.
 *
...
export function definePlace(opts: PlaceOptions = {}): Kind {
  return defineKind({
    ...
    // its kind registered: a build handed a different gazetteer must not fall
    // back to this one's capitals.
...
/**
 * The T0 build, and what `import { place } from "@smartput/country"` gets:
