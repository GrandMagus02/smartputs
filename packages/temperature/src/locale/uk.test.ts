...
import temperatureUk from "./uk";

const [readingUk, deltaUk] = temperatureUk;
const [readingEn, deltaEn] = temperatureEn;

const engine = () =>
  createEngine({
    locales: [composeLocale(ukrainian, temperatureUk)],
    kinds: [temperature, tempdelta],
  });

describe("temperature uk vocabulary", () => {
  test("ships one vocabulary per kind in the package", () => {
    expect(temperatureUk.map((v) => v.kind)).toEqual(["temperature", "tempdelta"]);
    for (const vocabulary of temperatureUk) expect(vocabulary.locale).toBe("uk");
  });
  ...
  test("covers every unit each kind declares", () => {
    ...
    expect(Object.keys(readingUk?.units ?? {}).sort()).toEqual(units(temperature));
    expect(Object.keys(deltaUk?.units ?? {}).sort()).toEqual(units(tempdelta));
  });

  test("every unit has aliases and a symbol (R8)", () => {
    for (const vocabulary of temperatureUk) {
  ...
  test("both kinds register the identical alias list, both scripts included", () => {
    for (const unit of ["c", "f", "k"]) {
      expect(deltaUk?.units[unit]?.aliases).toEqual(
        readingUk?.units[unit]?.aliases ?? [],
      );
    }
    // The Latin half is reused from the one `ALIAS` map in `units.ts` rather
    // than retyped, so a Ukrainian engine still reads "212 F"; the Cyrillic half
    // is appended.
    expect(readingUk?.units.c?.aliases).toContain("c");
    expect(readingUk?.units.c?.aliases).toContain("celsius");
    expect(readingUk?.units.c?.aliases).toContain("цельсій");
    expect(deltaUk?.units.k?.aliases).toContain("кельвінів");
  });

  // The per-unit decision is `en`'s, not re-taken here: nobody writes "20
  // градусів за Цельсієм" into a calculator, the spelled phrase is two tokens
  // and therefore unreadable, and the printer's spelled path only ever emits a
  // word the parser can read back. Asserting it against `en` rather than
  ...
  test("carries no forms on any unit, exactly as en carries none", () => {
    for (const [uk, en] of [
      ...
      for (const unit of ["c", "f", "k"]) {
        expect(uk?.units[unit]?.forms, `${uk?.kind}:${unit}`).toBe(
          en?.units[unit]?.forms,
        );
        expect(uk?.units[unit]?.forms, `${uk?.kind}:${unit}`).toBeUndefined();
      }
    }
  });
  ...
  test("the kinds themselves carry no Ukrainian word", () => {
  ...
  test("satisfies the locale contract", () => {
    expect(() =>
      assertLocaleContract(composeLocale(ukrainian, temperatureUk), [
        temperature,
        tempdelta,
      ]),
    ).not.toThrow();
    // The default counts are all integers, so they never reach CLDR's "other"
    // category at all. A fractional count is added for the same reason every
    // other uk vocabulary adds one — except that here it can only confirm the
    // absence of a `forms` table, since a unit with none is skipped before any
    // key is asked for. That is the honest shape of this kind's coverage.
    expect(() =>
      assertLocaleContract(
        composeLocale(ukrainian, temperatureUk),
  ...
  test("an engine built from it reads and writes Ukrainian temperature", () => {
    const e = engine();
    ...
    // invented eight Ukrainian keys here would print "2 кельвіни"/"5
    // кельвінів" — readable, and un-parseable in the spelled path.
    expect(e.evaluate("2 кельвіни").formatted).toBe("2K");
    expect(e.evaluate("5 кельвінів").formatted).toBe("5K");
    // The fractional row, which in every other uk vocabulary is the genitive
    // singular and here is again just the symbol — with Ukrainian's decimal
    // comma, which comes from CLDR through `numberFormat: "intl"`.
    expect(e.evaluate("1,5 кельвіна").formatted).toBe("1,5K");
    // Cyrillic in, Latin symbol out, for the two scale names Ukrainian writes as
    // degrees: `°C`/`°F` are what Ukrainian print uses, Latin letters included.
    expect(e.evaluate("20 цельсій").formatted).toBe("20°C");
    expect(e.evaluate("100 фаренгейт").formatted).toBe("100°F");
    // A conversion, read with the Ukrainian preposition "в" and printed in the
    // target scale. Both operands' words come from this file; the offsets come
    // from the kind.
    expect(e.evaluate("300 K в цельсій").formatted).toBe("26,85°C");
    expect(e.evaluate("212 F в C").formatted).toBe("100°C");
    // A conversion whose result groups: Ukrainian groups thousands with U+00A0,
    ...
    const diff = e.evaluate("30 цельсій - 20 цельсій");
    expect(diff.formatted).toBe("10°C");
    expect(diff.value?.kind).toBe("tempdelta");
  });

  // Three spellings a Ukrainian genuinely types and this vocabulary
  ...
  // cannot produce is dead weight that reads as coverage.
  test("records the Ukrainian spellings the lexer cannot reach", () => {
    const aliases = readingUk?.units.c?.aliases ?? [];
    ...
    // Ukrainian for the *second*, so claiming it here would make "20 с" a
    // temperature candidate in every composed engine to buy back a spelling the
    // "°" has already been thrown away from.
    expect(aliases).not.toContain("с");
    // "градус Цельсія" is two tokens, and a word token ends at a space, so no
    // alias can claim it. That is P5's `compoundSplitter`, not a missing word.
    expect(() => engine().evaluate("20 градусів цельсія")).toThrow();
  });

  // `ukrainian.selectForm` still answers for these units — it is a function of
  // the slot and the count, and knows nothing about which units have tables —
  // so the reason the two-axis grammar is unexercised here is the missing
  // `forms`, not a missing key. Pinning that keeps the previous test honest: it
  // asserts output does not move across the plural boundary, and this says why.
  test("selectForm still produces keys this kind has no table to index", () => {
    ...
    expect(readingUk?.units.c?.forms).toBeUndefined();
  });
  ...
  test("round-trips its own output", () => {
    ...
    // plain space before `lex()` sees it, so "9 032°F" comes back as two
