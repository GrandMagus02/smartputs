...
/**
 * One resolved place as a provider hands it back (spec §8).
 *
 ...
 * read (§3.1).
 *
 * Two fields the vendored tiers do not need. `admin1` is here because T2 is
 * where `paris texas` finally has a second Paris to choose between (§5.2), and
 * `postal` because a postal row is a place whose name is a code (§6.2). Both are
 * "" rather than optional, so a consumer never has to distinguish absent from
 * empty on data that is missing far more often than it is present.
 *
 ...
 * - `currency` is "" everywhere in this file. It is a country-level fact and
 *   `COUNTRIES` already holds it, so a caller joins on `country`; shipping a
 *   currency table inside the providers entry point would give it the data §3
 ...
 *   equal under §4.2's canonical identity, and that is the known cost. The
 *   rejected alternative was hashing the code into a synthetic id: it would look
 *   stable, survive into a Value's canonical, and collide with a real id.
 */
 ...
 * matcher's scope walks (§5.2) — a scope is a division or a country — so the
 * string a matcher claimed and the hint a provider takes describe one idea.
 */
...
/** The async half: the network lives behind this and nowhere else. */
 ...
 * Population and not §6.1's weights: those weights are the *matcher's*, computed
 * from a trie node's payload, and a provider row carries neither a capital flag
 * nor the country/city distinction they turn on.
 */
export function placeSnapshot(asOf: string, places: readonly Place[]): PlaceSnapshot {
  ...
  return Object.freeze({
    ...
    find(name: string, hint?: PlaceHint): Place | null {
      ...
      // Null rather than the unhinted winner. Spec §5.2 rules the other way for
      // the matcher — a scope selecting nothing is dropped — because there the
      // fold is destructive and a refused claim throws the whole input away.
