...
describe("speed es vocabulary", () => {
  ...
  test("an engine built from it reads and writes Spanish speed", () => {
    ...
    // `kph` has no forms.
    expect(engine.evaluate("10 nudos en kph").formatted).toBe("18,52 km/h");
  ...
  test("the compound Spanish writes is read through the kind's bridge", () => {
    ...
    expect(wired.evaluate("100 kilómetros / hora en kph").formatted).toBe("100 km/h");
    ...
    expect(wired.evaluate("60 millas / hora en mph").formatted).toBe("60 mi/h");
  });
});
