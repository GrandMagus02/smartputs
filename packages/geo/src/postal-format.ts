 ...
 * `postal.ts` answers while parsing, asked directly.
 *
 * **Which layer to reach for.** `PostalFormat` is the door. `PostalFormat.for("GB")`
 ...
 * book actually want. Reach past it to `postalAccepts` / `normalizePostal` only
 * when the row is one *you* brought — a `definePlace()` table, a row off the
 * wire — in which case `PostalFormat.of(row)` is still the nicer door; and to
 * `isBacktrackRisk` only if you are screening patterns of your own.
 *
 ...
 * `postal.ts` builds, so the de-anchoring, the case-insensitivity and the
 * empty-pattern guard in that file's `compile` are the only reading of
 * `CountryRow.postalRegex` there is. Re-spelling `compile` here was the obvious
 ...
 *
 * `postal.ts` does not export `compile`, so the reuse is by *driving* the
 ...
 * `postal.ts`'s qualifier branch reads "de 12345" as Germany's `12345` and would
 ...
 *
 * **What this file does decide**, because `postal.ts` has no opinion on it: where
 ...
 *
 * `CountryRow.postalRegex` is vendored data. It is regenerated from
 ...
 * answer in this package and it belongs to `postal.ts`. All 178 shipped formats
 ...
 * registry. `postal.ts` asks it to decide whether claiming a span would take a
 ...
 */
const testers = new WeakMap<CountryRow, ((code: string) => boolean) | null>();

function testerFor(row: CountryRow): ((code: string) => boolean) | null {
...
function buildTester(row: CountryRow): ((code: string) => boolean) | null {
  ...
  // other ways `postal.ts`'s `compile` gives up — the pattern does not compile,
 ...
 * `postal.ts`'s tokenizer cannot walk — three or more words, or a character that
 ...
 */
export function postalAccepts(row: CountryRow, code: string): boolean {
 ...
 */
export function normalizePostal(row: CountryRow, code: string): string | null {
 ...
 * 14, the regex, and `CountryRow` has no field for the other one. Deriving a
 ...
 * column means a generator change, a new `CountryRow` field and a new body hash,
...
for (const row of COUNTRIES) {
  byCode.set(row.a2, row);
  byCode.set(row.a3, row);
}

/** Keyed by row rather than by code so `of()` and `for()` share one instance. */
const instances = new WeakMap<CountryRow, PostalFormat>();
 ...
 * is no constructor: a `PostalFormat` for a country with no usable format is a
 * format that accepts nothing, which is a worse thing to hand a caller than
 * `null` — they check once at the door instead of discovering it a hundred codes
 * later — so `for` and `of` are the only ways in and both may refuse.
 */
export class PostalFormat {
  /** ISO 3166-1 alpha-2, uppercase — the form a caller passes back to `for`. */
  readonly country: string;
  /**
   * GeoNames' pattern, verbatim as `CountryRow.postalRegex` carries it.
   *
   ...
   * Not compiled and not de-anchored: it is the column, and it is `postal.ts`'s
  ...
  private constructor(row: CountryRow) {
    this.#row = row;
   ...
   * country, and when the country it names has no usable postal format — 74 of
   * the 252 rows, Antarctica and Tonga and the Cayman Islands among them, where
   * GeoNames' column is empty because there is nothing to put in it.
   *
   * Ireland is *not* one of them and is the near miss worth knowing: the Eircode
   * has been in the column since 2015, so `for("IE")` is a format and
   * `D02 AF30` validates.
   *
   * The same instance comes back for the same country, which is what makes the
   * matcher behind it worth caching.
   */
  static for(country: string): PostalFormat | null {
    const row = byCode.get(country.trim().toLowerCase());
    return row === undefined ? null : PostalFormat.of(row);
  }

  /**
   * The same door for a row you brought yourself — a `definePlace()` table, a
   * row off a provider. `null` on the same terms as `for`.
   */
  static of(row: CountryRow): PostalFormat | null {
