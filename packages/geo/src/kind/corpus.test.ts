...
const engine = createEngine({
  ...
  // the kind. `placeVocabulary(COUNTRIES)` for the same reason and one more — a `place` no
  ...
  locales: [composeLocale(en, [lengthEn, placeVocabulary(COUNTRIES)])],
  kinds: [
    number,
    length,
    definePlace({ countries: COUNTRIES, cities: CITIES, admin1: ADMIN1 }),
  ],
});
const corpora = await Corpora.load(new URL("../../corpus/kind/", import.meta.url), [
  {
    id: "en",
    engine,
  },
