 ...
 * proving is not any single country: it is that this file and `literal.ts` agree
 * about `PostalCountry.postalRegex` for every row, which is the defect the task
 * was written to avoid and the only one that could ship silently.
 */

/** The shipped table as a lookup — `@smartput/country` exports the same
 * instance as `POSTAL_FORMATS`, and this rebuilds it so the test stays a test of
 * this package. */
const FORMATS = new PostalFormats(COUNTRIES);

const rowOf = (a2: string): PostalCountry =>
  COUNTRIES.find((c) => c.a2 === a2) as PostalCountry;

const WITH_FORMAT = COUNTRIES.filter((c) => c.postalRegex.trim() !== "");
const formatOf = (a2: string): PostalFormat => FORMATS.for(a2) as PostalFormat;
...
test("for() takes alpha-2 or alpha-3 in any case, and hands back one instance", () => {
  expect(FORMATS.for("GB")).toBe(FORMATS.for("gb"));
  expect(FORMATS.for("gbr")).toBe(FORMATS.for("GB"));
  expect(FORMATS.for(" us ")).toBe(FORMATS.for("USA"));
  expect(FORMATS.for("zz")).toBeNull();
  expect(FORMATS.for("")).toBeNull();
});

test("of() is the same door for a row the caller brought", () => {
  const brought: PostalCountry = { ...rowOf("nl"), a2: "xx", aliases: [] };
...
test("a country with no postal system is null, not a format that takes anything", () => {
  ...
  for (const a2 of ["aq", "to", "ky"]) {
    ...
    expect(FORMATS.for(a2)).toBeNull();
...
test("a row whose pattern the screen refuses accepts nothing, and has no format", () => {
  ...
  // takes everything.
  const evil: PostalCountry = { ...rowOf("us"), a2: "xx", postalRegex: "^(a+)+$" };
