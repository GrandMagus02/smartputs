...
import { createPlaceFormatter } from "../kind/format";
import { createPlaceLiteral } from "../kind/matcher";
 ...
 * a strawman.
 */
const postalPlace = definePlace({ countries: COUNTRIES });
...
const engineOf = (kind: Kind) =>
  createEngine({
    // `placeVocabulary(COUNTRIES)` beside the built-ins: both kinds below are `place`, and a kind
...
/** Narrowed the way `createPlaceFormatter` narrows it: `LiteralMatch.meta` is optional
...
test("a code renders itself over its country's facts", () => {
  ...
  // `createPlaceFormatter` reads to decide it is looking at something inside a country
...
test("a code's unit is a registered country and its meta agrees", () => {
  ...
  for (const probe of ["SW1A 1AA", "M5V 3L9", "01310-100", "123 45", "us 90210", "90210"])
    for (const match of readings(probe)) {
      ...
      // and `createPlaceFormatter` reads exactly that to tell it from its country.
