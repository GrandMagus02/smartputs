...
// `RANK_STEP` is imported rather than respelled because readings have to be
// separated and not merely ordered, and the gap has to be the same one the name
// matcher keeps: §6.1's weights are a ranking the solver then scores against a
// single `ambiguityEpsilon`. Four countries share the British format and three
// share Sweden's `123 45`, so emitting them at one weight would turn `SW1A 1AA`
// — spec §2's "unambiguous shape" row — into an `AmbiguityError` naming Jersey
// and Guernsey. Ranking by the population *value* fails the same way, because
// the scale §6.1 uses for cities caps at `+2` and Czechia, Sweden and Slovakia
// all sit above the cap.
//
...
/**
 * The locale's keyword surface words, for the reason `matcher.ts` hardcodes the
 * same set: `in` is India, `to` is Tonga, `as` is American Samoa and `by` is
 ...
 *
 * Duplicated rather than imported: `matcher.ts` does not export it, and eleven
 * strings are cheaper than widening a neighbour's module to share them.
...
/**
 * `CountryRow.postalRegex` is GeoNames' column 14 verbatim, and it is not a
...
function shadowed(word: string, ctx: MatchCtx): boolean {
  return word.length < MIN_NAME_LENGTH && ctx.isUnitAlias(word);
}
 ...
 */
export function createPostalLiteral(countries: readonly CountryRow[]): LiteralMatcher {
  ...
  const meta = (row: CountryRow, code: string): PlaceMeta => ({
   ...
   * Angola, "are" the Emirates and "can" Canada, and `claimable` in `matcher.ts`
  ...
  const qualifier = (alias: string, code: string): PostalRow[] => {
    ...
    if (alias.length < MIN_NAME_LENGTH && !rows.some((e) => e.row.a2 === alias))
      return [];
