...
/**
 * Split a query into a country and a code, accepting exactly the two forms spec
 * §6.2 rules are one literal: `us 90210` and `90210 us`. The country is a token
 * of exactly two letters at either end, which is why `SW1A 1AA` survives intact
 * — neither of its tokens is two letters — and why `gb SW1A 1AA` also works.
 *
 ...
 */
function splitQuery(q: string): { code: string; country: string | undefined } {
...
/**
 * Postal codes from a mirror of the zauberware collection (spec §8), which is
 * GeoNames' own postal export republished per country as JSON, CC BY 4.0.
 *
 ...
 * reason `createLiveEngine` shares its in-flight fetch. A rejection deletes the
 * entry, so the failure reaches every waiting caller and the next call retries
 * instead of awaiting a settled rejection forever.
 */
export function postalCodes(opts: PostalCodesOptions): PlaceProvider {
  const doFetch = opts.fetch ?? globalThis.fetch;
  const files = new Map<string, Promise<Place[]>>();

  async function download(country: string): Promise<Place[]> {
    const url = opts.url
  ...
  function file(country: string): Promise<Place[]> {
    const key = country.toLowerCase();
    let pending = files.get(key);
    if (pending === undefined) {
      pending = download(key).catch((err: unknown) => {
        files.delete(key);
        throw err;
      });
  ...
  return {
    ...
    async lookup(q: string): Promise<Place[]> {
      ...
      return (await file(cc)).filter((p) => normalizeCode(p.postal) === wanted);
    },
  };
}
